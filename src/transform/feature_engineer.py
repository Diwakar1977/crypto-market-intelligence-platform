...
logger = Logger.get_logger("feature_engineer", "feature_engineer.log")
...
class FeatureEngineer:
    ...
    def _add_days_since_records(self, df: DataFrame) -> DataFrame:
        """Calculate days since ATH and ATL."""

        return df.withColumn(
            "days_since_ath", datediff(current_date(), col("ath_date"))
        ).withColumn("days_since_atl", datediff(current_date(), col("atl_date")))

    def _add_daily_volatility(self, df: DataFrame) -> DataFrame:
        """Calculate daily price volatility."""

        return df.withColumn(
            "daily_volatility_percentage",
            round(expr("try_divide(high_24h - low_24h, current_price)"), 2),
        )

    def _add_distance_from_ath(self, df: DataFrame) -> DataFrame:
        """Calculate normalized distance from all-time high."""

        return df.withColumn(
            "distance_from_ath", round(expr("try_divide(ath - current_price, ath)"), 2)
        )

    def _add_distance_from_atl(self, df: DataFrame) -> DataFrame:
        """Calculate normalize distance from all-time low."""

        return df.withColumn(
            "distance_from_atl", round(expr("try_divide(atl - current_price, atl)"), 2)
        )

    def _add_volume_market_cap_ratio(self, df: DataFrame) -> DataFrame:
        """Calculate trading volume to market-cap ration."""

        return df.withColumn(
            "volume_market_cap_ratio",
            round(expr("try_divide(total_volume, market_cap)"), 4),
        )

    def _add_supply_utilization(self, df: DataFrame) -> DataFrame:
        """Calculate circulating supply utilization."""

        return df.withColumn(
            "supply_utilization_pct",
            round(expr("try_divide(circulating_supply, max_supply)"), 2),
        )

    def _add_price_direction(self, df: DataFrame) -> DataFrame:
        """Classify 24-hour price movement."""

        return df.withColumn(
            "price_direction",
            when(col("price_change_24h") > 0, lit("UP"))
            .when(col("price_change_24h") < 0, lit("DOWN"))
            .otherwise(lit("FLAT")),
        )
